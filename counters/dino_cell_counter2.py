...
class DinoCellCounter2(BaseCellCounter):
    """
    DINO-based MSC counter with circular FOV masking and adjacency clustering.

    This class mirrors the feature-based logic of `DinoCellCounter`, then adds:
    - circular field-of-view filtering to remove detections in black corners
    - adjacency filtering to remove isolated detections
    - connected-component clustering where each connected group counts as one MSC
    """
    ...
    def count_cells(self, image: np.ndarray) -> tuple[int, list[tuple[int, int]]]:
        """
        Count MSC clusters and return `(count, coordinates)` in original image space.
        """
        all_detections: list[tuple[int, int, float]] = []
        debug_payload = None
        ...
        for scale in (0.75, 1.0, 1.25):
            ...
            detections = self._detect_peaks_with_radius(distance, binary_mask)
            all_detections.extend(
                self._rescale_detections_to_original(detections, scale)
            )
        ...
        detections = self._suppress_detection_overlaps(all_detections)
        detections = self._apply_circular_fov_mask(detections, image.shape[:2])
        clusters = self._cluster_adjacent_detections(detections)

        coordinates = [self._cluster_centroid(cluster) for cluster in clusters]
        self.debug_images = debug_payload or {}
        if self.debug:
            self.show_debug_images()

        self.cell_contours = [self._cluster_contour(cluster) for cluster in clusters]
        return len(coordinates), coordinates

    def run(self) -> int:
        """Load -> preprocess -> DINO features -> clustered MSC count."""
        self.load_image()
        self.preprocess()
    ...
    def show_debug_images(self) -> None:
        """Display debug intermediates for tuning."""
        for name, image in self.debug_images.items():
            cv2.imshow(f"dino2_{name}", image)
        cv2.waitKey(0)
    ...
    def _embed_tokens_with_interpolated_positional_encoding(
        ...
        """Use resized position embeddings for current patch grid."""
        x = patch_tokens
        cls_token = getattr(self.model, "cls_token", None)
        if cls_token is not None:
            cls_token = cls_token.expand(x.shape[0], -1, -1)
    ...
    def _detect_peaks_with_radius(
        self,
        distance: np.ndarray,
        mask: np.ndarray,
    ) -> list[tuple[int, int, float]]:
        """Find local maxima and estimate a radius for each detection."""
        if distance.max() <= 0:
            return []
        ...
        detections = []
        min_radius = math.sqrt(max(self.min_cell_area, 1) / math.pi)
        max_radius = math.sqrt(max(self.max_cell_area, 1) / math.pi)
        for y, x in peak_coordinates:
    ...
    def _apply_circular_fov_mask(
        ...
        """Discard detections whose centers lie outside the circular FOV."""
        ...
        kept = []
        for x, y, detection_radius in detections:
            distance = math.hypot(x - center_x, y - center_y)
            if distance <= radius:
                kept.append((x, y, detection_radius))
        return kept

    def _cluster_adjacent_detections(
        self,
        detections: list[tuple[int, int, float]],
    ) -> list[list[tuple[int, int, float]]]:
        """
        Remove orphans, then group connected detections into MSC clusters.
        Two detections are adjacent when center distance <= r1 + r2.
        """
        if not detections:
            return []

        adjacency = {index: set() for index in range(len(detections))}
        for i, (x1, y1, r1) in enumerate(detections):
            for j in range(i + 1, len(detections)):
                x2, y2, r2 = detections[j]
                if math.hypot(x1 - x2, y1 - y2) <= (r1 + r2):
                    adjacency[i].add(j)
                    adjacency[j].add(i)

        valid_indices = {index for index, neighbors in adjacency.items() if neighbors}
        visited = set()
    ...
    def _cluster_centroid(
        ...
        """Return the centroid of one connected cluster."""
        xs = [item[0] for item in cluster]
        ys = [item[1] for item in cluster]
        return (int(round(sum(xs) / len(xs))), int(round(sum(ys) / len(ys))))

    def _cluster_contour(
        self,
        cluster: list[tuple[int, int, float]],
    ) -> np.ndarray:
        """Create one contour covering all detections in a connected cluster."""
        points = []
        for x, y, radius in cluster:
            circle = cv2.ellipse2Poly(
                center=(int(round(x)), int(round(y))),
                axes=(max(2, int(round(radius))), max(2, int(round(radius)))),
                angle=0,
                arcStart=0,
                arcEnd=360,
                delta=15,
    ...
    def _suppress_detection_overlaps(
        self,
        detections: list[tuple[int, int, float]],
    ) -> list[tuple[int, int, float]]:
        ...
        for x, y, radius in detections:
            if 0 <= x and 0 <= y:
                if all((x - kx) ** 2 + (y - ky) ** 2 >= min_distance_sq for kx, ky, _ in kept):
    ...
    def _import_torch(self):
        ...
        try:
            ...
            raise ImportError(
                "PyTorch is required for DINO-based counting. Install `torch` and "
                "either `timm` or make cached DINO hub weights available."
            ) from exc
        return torch
    ...
    def _rescale_detections_to_original(
        ...
        return [
            (int(round(x / scale)), int(round(y / scale)), radius / scale)
            for x, y, radius in detections
        ]


# Lowercase alias requested by the task wording.
dino_cell_counter2 = DinoCellCounter2
